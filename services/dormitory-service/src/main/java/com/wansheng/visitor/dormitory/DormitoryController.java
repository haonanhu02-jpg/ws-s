...
@RestController @RequestMapping("/api/visitor/dormitory") class DormitoryController{
 private final DormitoryRepository repo;private final DormitoryScopeMode scope;private final Clock clock=Clock.systemUTC();DormitoryController(DormitoryRepository r,@Value("${visitor.dormitory.scope-mode:ACCOMMODATION_ONLY}")DormitoryScopeMode s){repo=r;scope=s;}
