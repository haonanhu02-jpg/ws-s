...
class EmployeeDormitoryService {
 ...
 List<Person> people(String name){return repo.people(name);} Person addPerson(PersonCommand c){return repo.person(repo.addPerson(c)).orElseThrow();} Person updatePerson(long id,PersonCommand c){found(repo.updatePerson(id,c));return repo.person(id).orElseThrow();} List<Stay> personStays(long id){repo.person(id).orElseThrow(()->bad("人员不存在"));return repo.staysByPerson(id);}
 ...
 @Transactional Stay book(BookCommand c,String op){repo.person(c.personId()).orElseThrow(()->bad("人员不存在"));Bed b=enabledBed(c.bedId());validateDates(c.plannedMoveIn(),c.plannedMoveOut());ensureAvailable(c.bedId(),c.personId(),-1,c.plannedMoveIn(),c.plannedMoveOut());long id=repo.book(c,op);if(Boolean.TRUE.equals(c.cleaningRequired()))repo.setBedCleaning(c.bedId(),true);repo.audit(id,"BOOK",null,b.id(),null,"BOOKED",op,null);return stay(id);}
 @Transactional Stay updateStay(long id,UpdateStayCommand c,String op){Stay s=stay(id);validateDates(c.plannedMoveIn(),c.plannedMoveOut());found(repo.updatePerson(s.person().id(),new PersonCommand(c.name(),c.centerName(),c.department(),c.gender(),c.category(),c.positionName(),c.rankName())));ensureAvailable(s.bed().id(),s.person().id(),id,c.plannedMoveIn(),c.plannedMoveOut());if(repo.updateStay(id,c,op)!=1)throw conflict("只有预订或入住中的记录可以编辑");repo.setBedCleaning(s.bed().id(),Boolean.TRUE.equals(c.cleaningRequired()));repo.audit(id,"UPDATE",s.bed().id(),s.bed().id(),s.status().name(),s.status().name(),op,"编辑入住信息");return stay(id);}
