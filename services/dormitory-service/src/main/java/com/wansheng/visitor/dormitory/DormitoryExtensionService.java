...
class DormitoryExtensionService {
 ...
 @Transactional List<FeeBill> generate(String month,String op){YearMonth current=validMonth(month),previous=current.minusMonths(1);FeeRule rule=repo.feeRule();if(!rule.enabled())throw conflict("费用结算规则尚未启用");for(var s:repo.feeSources(month,previous.toString())){BigDecimal water=usage(s.waterEnd(),s.previousWater()),electric=usage(s.electricEnd(),s.previousElectric());BigDecimal waterAmount="伏龙宿舍".equals(s.buildingName())?BigDecimal.ZERO:water.subtract(rule.freeWater()).max(BigDecimal.ZERO).multiply(rule.waterPrice()).setScale(2,RoundingMode.HALF_UP);BigDecimal electricAmount=electric.subtract(rule.freeElectric()).max(BigDecimal.ZERO).multiply(rule.electricPrice()).setScale(2,RoundingMode.HALF_UP);String occupants=String.join("、",repo.occupantNames(s.roomId(),current.atDay(1),current.plusMonths(1).atDay(1)));repo.saveBill(s,month,occupants,rule,water,electric,waterAmount,electricAmount,op);}return repo.bills(month);}
