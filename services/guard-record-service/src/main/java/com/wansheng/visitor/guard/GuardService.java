...
class GuardService {
  ...
  @Transactional GuardRecord create(GuardRecordRequest request,String op){validateVehicle(request);String id="GUARD-"+UUID.randomUUID().toString().replace("-","").toUpperCase(Locale.ROOT);GuardRecord record=new GuardRecord(id,request.visitorName().trim(),request.mobile().trim(),request.hostName().trim(),normalize(request.plateNumber()),request.vehicleEnteringFactory(),"NOT_STARTED",GuardStatus.WAITING_ENTRY,null,null,null,null);repo.createManual(record,op,clock.instant());return find(id);}
  @Transactional GuardRecord update(String id,GuardRecordRequest request,String op){validateVehicle(request);if(repo.updateDetails(id,request,op,clock.instant())!=1)throw new ResponseStatusException(HttpStatus.NOT_FOUND,"门卫记录不存在");return find(id);}
