...
onMounted(async()=>{if(!['ADMIN','SYSTEM_ADMIN'].includes(role))return;const r=await fetch('/api/visitor/admin/dashboard',{headers:{Authorization:sessionStorage.getItem('visitor-authorization')||''}});if(r.ok)stats.value=await r.json();else error.value='统计加载失败'})
</script>
