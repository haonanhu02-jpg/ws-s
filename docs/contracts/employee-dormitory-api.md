...
所有写接口从 JWT 用户名取得操作人，不接受客户端伪造操作人。附件限制 10MB，仅允许 PDF、图片、Word、Excel，并保存到持久化卷；费用按房间月度抄表结算，账单保存生成时的规则快照，确认后不可修改。多人同住房费分摊规则尚未确认，因此不自动拆分到个人。前端已支持批量导入模板、住宿/退宿确认单打印以及台账、人员、历史、抄表和统计数据导出 Excel。
